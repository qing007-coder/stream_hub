...
func NewEventSender(commonConf *config.CommonConfig, interactionConf *config.InteractionConfig) (*EventSender, error) {
	sender := new(EventSender)
	sender.events = make([]*storage.Event, 0)
	sender.duration = time.Duration(interactionConf.SendEventDuration) * time.Second
	sender.maxEvents = interactionConf.MaxEvent
